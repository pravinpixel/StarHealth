...
export const emailRegExp = (/([a-zA-Z0-9]+)([\.{1}])?([a-zA-Z0-9]+)\@(starhealth|starinsurance)([\.])in/g);
...
const PersonalInformationSchema = yup.object().shape({
  ...
  designation: yup.array().required("Enter your Designation").typeError().test("validate", "Enter your Designation", (value) => {
    if (value?.length !== 0) {
      return true
    }
  }),
  department: yup.array().required("Enter your Department").typeError().test("validate", "Enter your Department", (value) => {
    if (value?.length !== 0) {
      return true
    }
  }),
...
const UploadImagesSchema = yup.object().shape({
  passport_photo: yup.mixed().required("Upload Headshot image"),
  profile_photo: yup.mixed().required("upload Full sized photo"),
})
